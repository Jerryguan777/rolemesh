...
async def send_message(args: dict[str, Any], ctx: ToolContext) -> ToolResult:
    ...
    # binding; routing a fresh outbound message to that chat would
    # either be silently dropped (WebUI gateway doesn't subscribe to
    # internal bindings) or pollute parent UX. The delegate's reply
    # path is the RPC response, not this tool.
