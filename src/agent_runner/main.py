...
async def run_query_loop(
    ...
    tool_ctx = ToolContext(
        ...
        # here so downstream tool code never has to None-check.
        # ``dict(...)`` also makes a shallow copy — defensive against a
        # tool accidentally mutating ``init.role_config`` and bleeding
        # back into IPC state.
