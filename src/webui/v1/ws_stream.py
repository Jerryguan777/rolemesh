...
def _run_terminal_frame_or_none(
    ...
    read, no write, no inference. The chunk's own ``run_id``
    (container-attributed — echoed on output events, see
    AgentOutput.run_id) wins over the closure-tracked
...
async def stream(
    ...
    async def _forward_stream() -> None:
        ...
        carry their own (container-attributed) run_id which wins over
