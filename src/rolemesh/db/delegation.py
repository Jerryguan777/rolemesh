...
* ``create_child_conversation`` — INSERT ... ON CONFLICT for a sub-conv
  attached to the internal binding. Child exclusion from ``_state`` is
  enforced by the ``parent_conversation_id IS NULL`` list filter (see
  function docstring), not the removed ``requires_trigger`` column.
...
async def create_child_conversation(
    *,
    ...
    ★ The "child conv never enters ``_state``" invariant (§6 Step 2.5
    audit) is enforced on current main via the ``parent_conversation_id
    IS NULL`` filter in ``db.chat.get_conversations_for_coworker`` /
    ``get_all_conversations`` (``include_children=False`` by default) —
    the orchestrator's state loader funnels through those, so a child
    conv is never loaded and never processed by ``_message_loop``. (The
    legacy ``conversations.requires_trigger`` column this used to rely
    on was removed from the table upstream.) The loader-exclusion test
    in ``tests/core/test_loader_excludes_children.py`` guards it.
    ...
    async with tenant_conn(tenant_id) as conn:
        row = await conn.fetchrow(
            ...
            VALUES ($1::uuid, $2::uuid, $3::uuid, $4, NULL, $5::uuid, $6::uuid)
