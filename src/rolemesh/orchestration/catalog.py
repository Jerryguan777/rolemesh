...
super_agent bash perms; a "folder" label nudges the model into
...
def render_agent_catalog(
    ...
    *,
    ...
    Filters:
      ...
      - ``status == 'active'``
      - ``agent_role == 'agent'`` (super_agents are not delegation targets)
      - NOT ``is_frontdesk`` (frontdesks talk to specialists, not each other)
