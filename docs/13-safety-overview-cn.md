...
同样的输入可能产生不同的工具调用序列。无法像传统服务那样通过 "覆盖所有代码路径" 来获得高置信度——必须通过外部观测（policy、quota、audit）来约束行为边界。
...
**最佳实践**：
- **阻断高风险动作**——写操作、金钱、破坏性动作默认被阻断。Cursor、Claude Code、Devin 都有类似"危险命令拦截"
...
**最佳实践**：
- **完整 trace**——prompt、tool call、tool result、safety 决策全部持久化（immutable log）
- **结构化日志 + trace ID** 贯穿请求
...
├─────────────────────────────────────────────────────────┤
│ ★ 运行时策略框架         (block、PII、rate limit 等)     │ ← Safety Framework
├─────────────────────────────────────────────────────────┤
...
任何 control 失败、policy 不可用、网关不可达——**默认全部拒绝**而非降级放行。例外都需要显式 env 开关（如 `SAFETY_FAIL_MODE=open`）且日志告警。
...
可回滚 > 事前完美拦截。优先保留 soft delete、版本化、审计轨迹；不追求"100% 不可能误操作"，因为这种追求往往以易用性崩塌为代价。

### 5.5 Defense in depth

沙箱 + 网络 + 凭证 + 策略 + 审计，**任一层失守都不致命**。这就是为什么 Container Hardening / Safety Framework / Egress Control 是三个独立模块——任何一个有缺陷，其他两个仍然提供有意义的兜底。

### 5.6 默认阻断高代价动作

"不可逆 / 高代价"的动作不应不加检查地执行。一条安全规则可以携带一个阻断性判定（`require_approval`），让框架在调用执行前将其拦下，而不是放任 agent 继续。
