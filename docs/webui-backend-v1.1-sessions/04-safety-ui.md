...
> **Refresh 起源**：v1.1 最后一个 session。把以前所有 session 学到的 pattern 收拢应用——typed ApiClient（01a / 02a）、`raise_error_response`（01a）、`<rm-app-shell>` shell pack（00c）、RLS 双层防御（INV-1, 00b）、INV-5 lint pattern（03b）。安全 engine + DB 已运行多 phase，本 session 是**纯 UI/API 表层搬迁**。
>
...
6. **03b Findings § "INV-5 lint scope"** —— Python ↔ TS 一致 lint pattern
7. **01c Findings § "admin → v1 切换的实际范围"** —— lint:no-admin-chat allowlist 是 04 关注重点
8. 现有 admin safety endpoints (`src/webui/admin.py` 第 1196-1620 行附近)：
...
safety rules 的 read-only v1 是有意设计——写入仍走 admin surface。
...
- ❌ pre-existing TS errors in unrelated files（credentials-page / mcp-servers-page，独立 chore）
...
`cleanup_old_safety_decisions` — a documented retention sweep that
...
4. **Test the wire, not the helper.** Every new session that wrote `test_v1_*` exercised the JSON over httpx ASGI against a real Postgres testcontainer. The few times we mocked (e.g. a NATS publisher) we paid for it later via a follow-up that pinned the real protocol shape.

5. **Refreshes are cheap; rewrites are not.** Refreshing a session prompt cost ~10 minutes and prevented multiple scope spirals. The pattern: between sessions, read the upstream session's Findings, grep what they touched, refresh the next prompt's "Open questions" with locked answers. Refreshes that mattered: 02c (retire), 04 (GET-only + allowlist permanence).
