...
本文档描述 RoleMesh 的协调器（Orchestrator）和容器内的 Agent 如何通过 NATS 进行通信。内容涵盖原始方案存在的问题、为何选择 NATS、6 通道协议的设计，以及每个通道使用的 NATS 原语。
...
当 Orchestrator 想**主动关闭容器本身**时使用 —— 空闲超时、被更高优先级任务抢占、调度器在任务完成后驱动的关停。Agent 对请求做 ack；这个 ack 告诉协调器它现在可以放心停止 Docker 容器，不会截断尚在进行中的工作。

**为什么关停用请求-应答（Core NATS），而中断用 JetStream？** 关停信号在协调器端是和容器拆除明确绑定的 —— 延迟很重要，而且容器无论如何马上就要消失，所以持久性无关紧要。中断恰恰相反：容器还会继续运行，消息必须能熬过 agent 的繁忙时段，因此持久性比 ack 延迟更重要。
...
在启动容器之前，Orchestrator 会写入当前的定时任务快照，并预先按发起请求的 coworker 被允许看到的范围进行裁剪 —— 协调器负责切片，从而 agent 拿到的就是一份现成可读的列表。Agent 通过 `list_tasks` MCP 工具读取它。权限语义在 `auth-architecture.md` 中详述。
...
- `web.>`（`web-ipc` 流）—— FastAPI 与协调器之间的 WebUI 流量
- `approval.decided.*` / `approval.cancel_for_job.*`（`approval-ipc` 流）—— 审批模块的 worker 队列与 Stop 级联
- `egress.{rules,identity,mcp}.snapshot.request` —— egress 网关在启动时调用协调器的请求-应答 RPC
...
协调器为 agent IPC 维护一个流；另外两个流（`web-ipc`、`approval-ipc`）与它共存于同一台 NATS server，但分别由 WebUI 和审批模块拥有 —— 它们在各自模块的文档中描述。
...
IPC 层关于授权的契约只有一句话：**通道 4 / 通道 5 的负载携带 `tenantId` + `coworkerId`，但绝不携带发起请求的 agent 的权限。** agent_runner 从 `AgentInitData` 中（而不是从 LLM 中）填入这些字段，协调器在响应请求之前会查找该 coworker 的权威 `AgentPermissions`。Agent 无法通过修改负载来提权，因为负载本来就不声明权限。通道 6 的快照同样在协调器侧预先做了过滤，因此即便是有 bug 的 `list_tasks` 调用也无法读取另一个租户的数据。
