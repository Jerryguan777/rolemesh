...
RoleMesh 最初只有一个布尔字段：`is_main`。Admin Coworker 可以做任何事情——查看所有任务、管理其他 agent 的调度、注册新会话。非 admin 的 Coworker 在自身范围之外什么也做不了。
...
| **完整 RBAC**（角色 + 权限 + 资源） | 灵活性最大化 | 对仅有 3 项能力来说严重过度设计。组合爆炸。难以推理。 |
| **平铺的能力位** | 易于理解，覆盖真实用例，无抽象开销 | 无法表达深度嵌套的策略（但我们也不需要） |

我们选择了 **平铺的能力位**。一个 agent 携带三个布尔值，默认全部为最小权限（全 `false`）；每一项都需显式授予。这里没有角色模板——早先的 `super_agent`/`agent` 角色与 `data_scope` 维度作为过度设计被移除（角色只是打包了默认值，而 `data_scope` 既充当宿主文件系统挂载开关，又是一个与沙箱隔离模型相冲突的全租户数据窗口）。

历史遗留的 `is_admin` 列与 `agent_role` 列都已被完全移除。唯一的权威字段是权限 JSONB。
...
| 权限 | 默认值 | 控制对象 |
|-----------|:---:|---|
| `task_schedule` | `false` | 是否能创建调度任务（cron、interval、once）。 |
| `task_manage_others` | `false` | 是否能 暂停/恢复/取消/更新 其他 agent 的任务。**同时**赋予在快照中看到其他 agent 任务的可见性——管理需要先能看见。 |
| `agent_delegate` | `false` | 是否能调用其他 agent（为将来的 frontdesk agent 预留；尚未启用）。 |
...
权限作为一个 JSONB 列存储在 `coworkers` 表中。它们通过 `AgentInitData`（NATS KV bootstrap 载荷——见 [`2-nats-ipc-architecture.md`](2-nats-ipc-architecture.md)）流入正在运行的 agent。IPC 契约一句话即可概括：**载荷只携带 `tenantId + coworkerId`，但永不携带权限本身；orchestrator 在响应任何 Channel 4 / Channel 5 请求之前，会查找该 Coworker 的权威权限**，所以 agent 无法通过编辑载荷来提权。

反序列化时缺失或为空的权限会被强制收敛为最小权限默认值（全 `false`）。
...
`src/webui/auth.py` 对每个 REST 和 WebSocket 处理器，通过 `AuthProvider.authenticate()` 验证请求 token。dev/test 的 `BOOTSTRAP_USERS` 快捷通道和 OIDC PKCE 流程都在此处接入。每条路径产出的 `AuthenticatedUser` 的 `user_id` 都是真实 UUID——external JWT provider 在边界强制这一点（非-UUID 的 `user-id` claim 会被拒绝），因此审计 / `created_by` 外键写入无需再为伪用户做兜底。各种表面细节（涉及哪些路径、刷新如何处理、`?token=` 查询参数的语义）见 [`5-webui-architecture.md`](5-webui-architecture.md)。
...
在 orchestrator 的消息分发路径上：所有 agent 只能向自己的会话发送消息。这里没有 admin 后门——没有任何权限能解锁跨会话发消息，因为这是一项架构选择，而不是一项权限（参见 "Agent 权限中 _不_ 包含的内容"）。

### 4. 容器构建器

`src/rolemesh/container/runner.py:build_volume_mounts()` 永远不会把宿主项目根目录挂载进任何容器。快照可见性由 `task_manage_others` 管控：
...
没有 `task_manage_others` 的 agent 在快照中只能看到自己的任务——orchestrator 会预先过滤 Channel 6 的快照，因此即便有缺陷的 `list_tasks` 调用也读不到其他 agent 的数据。
...
| `coworkers` | agent 定义（`permissions` JSONB、tools、container_config） |
...
Schema 机制（默认租户的创建、幂等的 `_create_schema()` 形态）在 [`4-multi-tenant-architecture.md`](4-multi-tenant-architecture.md) 中描述。

## Admin API

`src/webui/admin.py` 在 `/api/admin/` 下暴露 RESTful endpoint，用于租户、用户、agent、绑定、会话和任务的管理——由用户角色检查保护。完整接口表面（存在哪些 endpoint、各由哪个模块拥有）记录在 [`5-webui-architecture.md`](5-webui-architecture.md) 的 "Beyond chat: Admin surface" 一节。
...
| `src/rolemesh/auth/authorization.py` | 纯 auth 函数：`can_schedule_task()`、`can_manage_task()`、`can_delegate()` |
