...
| **rev4** | `be953c5` | 评审修正：gateway 降级启动（修启动死锁）、DNS 端口机制修正、补遗漏（remote_control 删除 / webui 镜像 / mount-allowlist / resolver 上游 / 测试 churn）、deny 探针下放 |
...
预期净效果：删除约 1,200 行命令式 provisioning / 视角翻译 / 死代码，
换来 ~150 行 compose YAML。启动时序问题**不是消失而是显式化**（rev4
修正措辞）：从"隐式的代码执行顺序"变为"显式的启动契约"，其代价是
gateway 必须新增降级启动语义（§4.6）——rev3 宣称"#79 类 bug 整类消失"
是错误的，依赖只是换了方向，必须正面解决。
...
| `evaluation/cli.py:326-338` 的 `ensure_gateway_running_and_register_dns` 自举调用 | eval 独立进程自启 gateway | 改 verify-only（基础设施由 compose 保证） |
...
| `orchestration/remote_control.py`（~250 行 + 测试） | 在宿主机 spawn `claude remote-control` 子进程 | **删除**（已拍板 2026-06-12）：`start_remote_control` 入口零调用属死代码；其本意（远程操作宿主机）被容器化在概念上消解，保留即虚假承诺 |
...
| 启动顺序 | gateway 仅 `depends_on` NATS；orchestrator 的 verify 重试 gateway healthz | readiness probe + verify 重试（同左语义） |
| gateway resolver 上游 | Docker embedded DNS（127.0.0.11） | kube-dns（读 gateway 自身 resolv.conf） |
...
- **Docker 版**：agent-net 存在且 `Internal=true`、egress-net 存在、
  ...
  地址一致、NATS 可达。
- **K8s 版**（rev4 调整）：四条 NetworkPolicy 存在且 selector 命中
  （对象级校验，毫秒级）、PVC Bound、gateway Service/healthz、RBAC 自检
  （SelfSubjectAccessReview）。**deny 实测**（探针 Pod 出网必须失败，防
  CNI 不支持 NetworkPolicy 的假绿）不在每次启动跑——探针 Pod 的调度+
  拉镜像会把启动拖到十秒级，滚动更新难受；下放到 **helm test**（安装时
  一次）与契约测试 T-NET。代价：安装后更换 CNI 的窗口不设防（极罕见
  操作），NOTES 注明。
...
- evaluation CLI（仅 docker 模式，已拍板）：作为一次性容器
  ...
  其 gateway 自举调用改 verify-only（§1.2 第一组）。
- **webui 镜像**（rev4 补遗）：`src/webui` 是独立 FastAPI 进程，需自己的
  Dockerfile（multi-stage：node 构建 `web/dist` → 运行层），
  `WEB_UI_DIST` 改绝对路径/env 注入。计入 P1（~100 行）。
- **mount-allowlist**（rev4 补遗）：`MOUNT_ALLOWLIST_PATH` 从
  `~/.config/rolemesh/...` 改为 env 可配；compose bind
  `~/.config/rolemesh`（ro）/ K8s 用 ConfigMap。
- 删除 `orchestration/remote_control.py`（已拍板）：`start_remote_control`
  零调用属死代码；其本意（远程操作宿主机）被容器化消解。`main.py:1816`
  的 `restore_remote_control()` 调用与测试一并删除。

### 4.6 gateway 降级启动（rev4：修启动顺序死锁）

**问题**（评审发现，已代码核实）：gateway 启动即经 NATS 拉取规则快照，
拿不到则拒绝启动（`gateway.py:147-150` "refusing to start"）；快照的
responder 在 orchestrator 里（`main.py:1707-1713` 注释明确 gateway 必须
后于 responders）。rev3 的 `orchestrator depends_on gateway(healthy)`
与之构成循环依赖：gateway 等快照，快照等 orchestrator，orchestrator 等
gateway healthy——死锁。

**修正**：gateway 改为降级启动——
- healthz 在 NATS 连通后即返回 200，不等快照；
- 规则面初始姿态 **default-deny**，后台重试快照直至成功后切换为正常
  策略（沿用 MCP registry 快照已有的 graceful 模式——
  `gateway.py:182-193` "instead of crash-looping over a transient
  NATS"——把"空注册表"语义换成"全拒"）；
- compose：gateway 仅 `depends_on` NATS；orchestrator 的
  `verify_infrastructure` 带超时重试等 gateway healthz；
- 快照未就绪窗口内 agent 流量被全拒——可接受：该窗口内 orchestrator
  尚未完成启动，不存在 agent。

估算 ~120 行（gateway.py + 测试），**计入 P1**。
...
（rev4 修正：rev3 的"两边统一 1053"在 Docker 侧**不成立**——compose 的
`ports:` 只作用于宿主发布，容器互访直达实际监听端口；且
`HostConfig.Dns`/resolv.conf 没有端口语法。按 §3 的机制映射哲学各用各的：）

- **Docker**：resolver 监听 53，保留 `NET_BIND_SERVICE` 单 cap；
- **K8s**：容器监听 1053，Service port 53 → targetPort 1053，
  namespace 全量 PSA restricted。

agent 的 nameserver 指向 `EGRESS_GATEWAY_DNS_IP`（静态配置）。

**上游策略**（rev4 补遗，P2 工作项）：agent 解析内部服务名（`nats`、
`egress-gateway`）同样经过 gateway resolver，其递归上游两边不同：
Docker 转发 Docker embedded DNS（127.0.0.11），K8s 转发 kube-dns
（从 gateway 自身 resolv.conf 读取——gateway 用默认 dnsPolicy）。
契约用例 T-NET 锁定：agent 经 gateway 能解析 `nats`；非白名单域名
NXDOMAIN。
...
**DooD 语义变化（rev4 补遗）**：additional_mounts 的任意宿主路径
（`~/projects` 之类）在 docker 模式仍可挂——bind 源由**宿主 dockerd**
解释——但 orchestrator 在容器内无法再做存在性检查；且 bind 源不存在时
dockerd 会**静默创建 root 属主的空目录**（比"验不了"更危险的坑）。
处置：mount_security 对此类路径降级为模式校验 + 显著日志警示，文档写明；
K8s 下此类挂载完全不可用，必须走 `agent.extraVolumes`。
...
tests/container/contract/
  ...
  test_network.py      # T-NET-*: agent 无直接外网/裸 IP metadata 不可达/
                       #          DNS 只能经 gateway/经 gateway 可解析内部
                       #          服务名(nats)/无 token 请求 407/deny 实测
...
**起点说明**（rev4）：main 上已有 `docker-compose.dev.yml`，且 agent-net
已声明归 compose 所有——声明化已走半步，以它为底座扩展，不从零新建。

### 8.2 生产：Helm chart（`deploy/charts/rolemesh/`）

结构沿袭 rev2（orchestrator 单副本 Deployment **`strategy: Recreate`**
——单副本有状态，滚动更新瞬间双副本会出问题（rev4）+ RBAC 仅 pods、webui、
...
删除 `remote_control.py`（死代码，2026-06-12）。
...
| 阶段 | 内容 | 验收 |
|---|---|---|
| **P1 声明化 + 解耦** | compose 文件 + orchestrator/webui Dockerfile；**gateway 降级启动**（§4.6）；删第一、二组代码（§1.2，含 remote_control）；`verify_infrastructure`（docker 实现 + k8s NotImplemented）；挂载翻译层；`compute_egress_routing` 单路径化；存量测试改造（`EGRESS_CONTROL_ENABLE` monkeypatch 8/7/4 处 + attack_sim + container/integration 拓扑假设）；契约测试框架 | docker 模式契约测试全绿；`docker compose up` 端到端跑通一条 agent 会话；**开发内环硬验收**（改 src→reload 生效体感、debugpy 断点可用）；净删码 |
...
| 阶段 | 估算 | 备注 |
|---|---|---|
| P1 | 新增 ~1,000（compose 150 + verify 150 + 翻译层 80 + gateway 降级 120 + webui 镜像 100 + 契约测试 docker 侧 ~400）+ 存量测试 churn ~400；**删除 ~1,400**（含 remote_control ~300） | 生产代码净负 |
| P2 | ~1,250（k8s_runtime 650–800 + verify-k8s 100 + resolver 上游 ~50 + kind 150 + k8s 测试 fixture 300） | 较 rev2 略降（无 provider/policy 三件套） |
