...
本文档说明 RoleMesh 基于浏览器的 WebUI 通道的设计——为什么它是一个独立进程、它如何通过 NATS 与协调器通信，以及这些决策背后的权衡。

## 背景

RoleMesh 支持 Telegram 和 Slack 作为消息通道。每一种都有一个网关（TelegramGateway、SlackGateway），实现 `ChannelGateway` 协议并运行在协调器进程内部。这种方式之所以可行，是因为 Telegram 和 Slack 网关是事件驱动的监听器——它们从外部 API 接收消息，然后调用协调器的回调。

新增一个基于浏览器的 WebUI 引入了不同的挑战：WebUI 需要一个 **HTTP/WebSocket 服务器**，供浏览器连接。这个服务器必须处理实时流式传输、静态文件服务、REST API（会话历史、管理面板）以及真实的认证流程——这些表面并不适合放在进程内的网关形态里。
...
WebSocket 服务器运行在协调器进程内部，与 Telegram/Slack 网关相同。
...
**缺点：**
- 把 HTTP 相关问题（路由、中间件、认证、静态文件）耦合进协调器
- 无法独立于协调器伸缩 WebUI
- 难以嵌入已有的 SaaS 平台——它们需要的是标准的 REST/WebSocket API，而不是函数调用接口
- 为未来功能（会话管理、管理面板）增加 REST API 会让协调器膨胀
- 没有 OpenAPI 文档供第三方集成
...
FastAPI 作为独立进程运行。与协调器的通信完全通过 NATS。

**优点：**
- 清晰的职责分离——协调器处理代理生命周期，FastAPI 处理 HTTP
...
- 未来的 REST API（会话管理、计费、管理面板）自然地落在这里
...
我们选择了选项 B，因为 RoleMesh 的设计目标是 **agent-as-a-service 平台**。WebUI 只是 API 的其中一个消费者——未来的消费者还包括第三方 SaaS 集成、移动应用和管理面板。一个良好定义的 API 边界值得这部分额外的复杂度。

## 为什么用 NATS（而不是直接读写数据库）？

FastAPI 服务可以完全绕开 NATS，直接写入数据库，再让协调器轮询新消息。我们拒绝了这个方案，原因是：

1. **延迟**——轮询会增加数秒级延迟。NATS 在毫秒级内就能投递消息。
2. **流式传输**——代理的输出必须实时流到浏览器。协调器通过 NATS（`agent.{job_id}.results`）接收代理的分块输出，必须立即转发出去。流式分块没有数据库表可供轮询。
3. **一致性**——协调器管理会话状态、session ID、代理调度和容器生命周期。如果 FastAPI 直接写数据库，就会绕过所有这些逻辑，并产生竞态条件。
4. **一致性对齐**——协调器已经使用 NATS 进行代理 IPC。把 web 通道的 subject 加进同一套基础设施是自然的。

FastAPI 的确直接读取数据库，但仅用于一件事：**token 校验和会话历史读取**（通过 RLS 绑定到发起请求的租户）。这是只读路径，不会干扰协调器的权威状态。
...
| `web.typing.{binding_id}.{chat_id}` | Orchestrator → FastAPI | 代理开始/停止处理 |
| `web.outbound.{binding_id}.{chat_id}` | Orchestrator → FastAPI | 代理的完整回复 |

### 为什么用独立的流？

我们使用独立的 `web-ipc` 流，而不是把 subject 加进已有的 `agent-ipc` 流，原因是：

- **不同的保留需求**——代理 IPC 的消息是瞬时的（被协调器消费一次）。Web 消息可能需要不同的保留策略，用于调试或回放。
- **不同的消费者**——代理 IPC 仅由协调器消费。Web 的 subject 由 FastAPI 消费。独立的流可以避免消费者偏移量被相互污染。
- **运维清晰度**——`nats stream info web-ipc` 可以立刻看到 web 通道的健康状况，而不会与代理流量混在一起。
...
在协调器侧，`WebNatsGateway` 满足与 `TelegramGateway` 和 `SlackGateway` 相同的 `ChannelGateway` 协议。
...
经过网关层之后，协调器对这三种网关的处理方式完全相同——同样的消息路由、会话查询、代理调度和输出处理。

### 流式传输：为什么新增方法？

`ChannelGateway` 协议定义了 `send_message(binding_id, chat_id, text)`，用于发送一段**完整**的文本。对 Telegram 和 Slack 来说这是正确的——你只发送一条带有完整响应的消息。

对于 WebUI，我们想要**流式**——浏览器应该看到文本随着代理生成而出现，而不是等待整段响应。为了在不修改 Telegram/Slack 所用协议的前提下支持这点，`WebNatsGateway` 增加了两个额外方法：
...
该 token 会被放入代理的 `AgentInitData` 中转发给协调器，使 MCP 工具调用可以把用户身份带到下游——参见 [`auth-architecture.md`](auth-architecture.md) 和 [`external-mcp-architecture.md`](external-mcp-architecture.md)。
...
IdP 签发的 token 会被镜像到主机侧的 `TokenVault`，使协调器（以及外部 MCP 服务器）可以代表用户调用 API，而不会把刷新材料暴露给代理容器。

### 3. 已存储 token 的回放

如果 OIDC 流程已把一个 token 存入 `sessionStorage` 且尚未过期，SPA 会无需重新登录就解析认证（参见 `app.ts:resolveAuth()`）。

完整的身份模型——用户角色、代理权限、IdP 集成选择——在 [`auth-architecture.md`](auth-architecture.md) 中。WebUI 进程是这些概念的传输层；它并不拥有它们。

## 不仅仅是聊天：管理面板

WebUI 进程不只是一个聊天面板。同一个 FastAPI 应用还提供平台的 REST + UI 管理面板。把管理端点放在这里（而不是协调器中）是"为什么用独立进程"决策的直接延伸：HTTP 相关的事情就应该在 HTTP 这一侧。

管理面板按模块分组——每一组的权威文档都在自己的文件中：

| 表面 | 端点 | 归属文档 |
|---|---|---|
| 会话历史 | `GET /api/conversations`、`GET /api/conversations/{chat_id}/messages` | 本文件 |
| Coworker / 代理 CRUD | `/api/admin/agents/*`（含每个代理下嵌套的 skills CRUD） | [`auth-architecture.md`](auth-architecture.md)、[`skills-architecture.md`](skills-architecture.md) |
...
| 决策 | 已选 | 备选 | 原因 |
|---|---|---|---|
| 进程模型 | 独立的 FastAPI | 嵌入到协调器中 | SaaS 集成、API 优先、独立伸缩 |
...
- [`auth-architecture.md`](auth-architecture.md) —— `AuthProvider` 抽象、代理 + 用户权限、OIDC 细节
- [`nats-ipc-architecture.md`](nats-ipc-architecture.md) —— 协调器侧的 NATS 协议；`web-ipc` 流与 `agent-ipc` 并列存在
...
- [`skills-architecture.md`](skills-architecture.md) —— 每个代理 skills CRUD 背后的端点
