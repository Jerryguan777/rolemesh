...
它涵盖为什么这是一个**框架**而不是若干硬编码 check、考虑过哪些抽象层级并为何被否决、Stage / Context / Check / Rule 四件套的设计意图、与现有 Approval 模块的关系，以及 V1 故意没做的事。

目标读者：将来添加新 Check（PII 检测器、prompt injection classifier、egress 规则等）、调试某条规则为何没触发、把 Safety 移植到新 stage、或理解"为什么不用 OPA/CEL"的开发者。前置阅读：[`13-safety-overview.md`](13-safety-overview.md) §2.4 / §2.5 / §2.7 / §2.8、[`12-approval-architecture.md`](12-approval-architecture.md)。
...
8. **不替换现有 Approval。** Approval 是 Safety 的一种 "action 类型"（V2 桥接），不是把现有 approval 状态机吞掉重写。
...
**缺点**：散落的 fail-mode、不一致的审计、改一种检测可能影响其他检测的命中顺序、加新检测要改主流程代码。**和 [`12-approval-architecture.md`](12-approval-architecture.md) 模块要的"声明式策略"形态完全冲突**——admins 没法不写代码就配规则。
...
- 审计 / 多租户 / approval 工作流 / NATS IPC 都要深度定制——最后还是要包一层
...
**优点**：所有目标都满足；新增能力 = 写 Check 类 + 注册一行；不引入 DSL；与现有 Approval 模块平行而非冲突。
...
[`12-approval-architecture.md`](12-approval-architecture.md) 描述的 Approval 是 **Safety 的一种 action 类型**——V2 桥接，**不替换**：

- Approval 模块的状态机、人工审批 UX、approver 解析、批量审批工作流——全部保持独立
- Safety V2 引入 `action="require_approval"` 时，pipeline 不持有 approval state、不等待、不轮询；它只是"发起人"——发一条特殊 NATS 事件，orchestrator 收到后调 `ApprovalEngine.create_request(...)`
- 容器侧看到的就是一个 `block`（"等待审批中..."）
- 审批通过后由 Approval 模块的现有执行回路处理

这种**桥接而不合并**的设计避免：
- Safety 框架变成 approval 引擎的 fork
- approval 工作流（已有 14 个 E2E 测试钉死）被 safety 改造拖累
- "策略"和"审批"两个语义混在同一张表
...
- `require_approval` action 桥接 Approval 模块
...
- **替换现有 Approval / sender_allowlist / mount_security**——它们继续独立存在
...
- **审批工作流** → Approval 模块（Safety V2 桥接，不合并）
