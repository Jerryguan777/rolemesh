# Frontdesk Architecture — v1.2

> Status: Production. Branch `feat/frontdesk` (9 commits) merging to `main`.
> Spec: `docs/frontdesk-impl/handbook.md` is the source of truth for the
> implementation contract; this document is the user-facing architecture
> overview.
...
`is_frontdesk=TRUE` is only valid alongside `agent_role='super_agent'`.
The admin UI enforces the invariant; there is no DB CHECK constraint
because future operator workflows (e.g. demote-then-flip) need the
flexibility. `routing_description` is a domain-agent capability card
read by the frontdesk LLM when routing (max 500 chars).
...
`GET /api/admin/approvals?conversation_id=<parent>` walks the parent:
