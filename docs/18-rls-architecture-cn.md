...
- **触发器派生的列**（如 `safety_rules_audit.tenant_id`）如果触发器被禁用、或新写入路径绕过它，就会漂移。
...
- 应用代码的租户意图变得不透明（"为什么这条查询读 `safety_decisions` 但根本看不到任何租户上下文？"）。
...
```
┌─────────────────────────────────────────────────────────────┐
│   App Pool   (角色: rolemesh_app,  NOBYPASSRLS)                 │
│   ├── 用于：webui REST handler、NATS 业务 handler            │
│   └── 包装为：tenant_conn(tenant_id) 上下文管理器             │
├─────────────────────────────────────────────────────────────┤
│   Admin Pool (角色: rolemesh_system, BYPASSRLS)                   │
...
| 类别 | 连接 | 签名 | 返回 | 例子 |
|---|---|---|---|---|
| **A. 租户内业务** | `tenant_conn(tenant_id)` | `tenant_id` 必填 kwarg | 完整 row | `get_coworker`、`list_safety_rules` |
| **B. 跨租户维护** | `admin_conn()` | 无 `tenant_id` 参数 | 含 `tenant_id` 字段的行，供下游分发 | `list_due_scheduled_tasks`、`cleanup_old_safety_decisions` |
| **C. 租户解析器（边界 bootstrap）** | `admin_conn()` | 无 `tenant_id`（输出即权威） | **最小化标量**（str / tuple） | `resolve_coworker_tenant`、`resolve_user_for_auth` |
...
- **NATS legacy fallback**。像 `agent.<job_id>.results` 这样的 subject 可能在 body 里没有 `tenant_id`（比如某条消息是协议加 tenant_id 之前发的）。executor 必须先解析出 job 所属的租户，才能做任何 RLS 作用域的工作。
...
- **`safety_rules_audit`** 有一个去规范化的 `tenant_id` 列（通过插入触发器从 `safety_rules` 复制），以及一个复合外键 `(rule_id, tenant_id) → safety_rules(id, tenant_id)`。触发器让写入保持便利；复合外键即便触发器被禁用也能在 DB 层阻止漂移。读热路径使用复合索引 `(tenant_id, rule_id, created_at)` —— 前两列索引 seek，再按 created_at 排序扫描。
...
- **`external_tenant_map`** 是 OIDC 租户查找表；`rolemesh_app` 对它没有任何权限。
...
当应用代码调用 `get_safety_rule(rule_id, tenant_id="错的")`，函数返回 `None`。这和"规则不存在"无法区分。这个行为是**故意**的 —— 区分两者会让攻击者能探测其它租户的资源是否存在。

代价是调试。一个 endpoint 有 bug、传错 `tenant_id`，表现是"找不到"，让开发者跑去查错的地方。缓解措施是：最安全敏感的几个 by-id 函数（`get_safety_rule`、`get_user`、`list_safety_rule_audit`）内部用 CTE 模式检测不匹配，并发出结构化告警日志和 `tenant_mismatch_attempted` 指标 —— 对运维可观测，但不暴露给调用方。
...
`safety_rules_audit.tenant_id` 由插入触发器填充，不是应用代码。触发器可能被 DBA 静默禁用，新写入路径也可能绕过触发器而不写这一列。复合外键 `(rule_id, tenant_id) → safety_rules(id, tenant_id)` 抵御两者：含 NULL 或错误 `tenant_id` 的行根本插不进去。触发器保留作为便利；外键是安全网。

### 双 Pool vs. 单 Pool

单 pool 配 `SET ROLE rolemesh_app` / `SET ROLE rolemesh_system` 切换能节省内存和连接槽位。风险 —— 忘记 reset 角色把 admin 权限泄漏到下个请求 —— 对当前的风险等级是不可接受的。多一个 pool 的成本是几个连接；这是一种廉价的物理隔离。
...
2. **基础设施**。创建 `current_tenant_id()` SQL 函数、`rolemesh_app` / `rolemesh_system` 角色、双 pool 和 `tenant_conn` / `admin_conn` 包装。RLS 仍未启用；业务行为不变。
3. **连接迁移**。把每个 `pool.acquire()` 替换为 `tenant_conn(tenant_id)` 或 `admin_conn()`，按函数类别决定。本阶段后所有业务路径都正确携带租户上下文 —— 但 RLS 仍未强制。
4. **逐表启用**。一次一张表启用 RLS，从 blast radius 最小的（`safety_rules_audit`）开始作 canary，最后是 `users` 和 `oidc_user_tokens`。每张表一个 commit，可单独用一条 `ALTER TABLE ... DISABLE ROW LEVEL SECURITY` 回滚。
...
- **不防御 `rolemesh_system` 凭据泄漏**。任何掌握 admin 角色的人都拥有完整的跨租户访问。该角色的凭据必须像 Postgres superuser 一样小心管理。
...
4. 角色是 `rolemesh_app` 还是不小心走到了 `rolemesh_system`？连接里跑 `SELECT current_user`。
...
- [`15-safety-framework-architecture-cn.md`](15-safety-framework-architecture-cn.md) —— safety rule audit schema 和同步 `tenant_id` 的触发器
- [`2-nats-ipc-architecture-cn.md`](2-nats-ipc-architecture-cn.md) —— NATS subjects 和 `resolve_coworker_tenant` 服务的 legacy fallback
