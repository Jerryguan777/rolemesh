...
v1.1 完工后 chat + 配置 + skills + safety 都打通，但 UI 仍是**扁平 sidebar**——chat、coworker 配置、零件管理、治理、observe 全平铺在一个层级。v2 重新设计 IA：把 **operate / configure / observe** 分成三个独立壳，coworker 提升为一等公民（成品），其它都是装配它的零件。

v1.1 是后端 + API + 协议骨架，v2 是**纯前端重构 + UX 重设计**——不动 schema、不加 endpoint、不变协议。

## Sources of truth（与 v1.1 一致的双轨）

- **数据契约**：`contracts/openapi.yaml` + `web/src/api/generated/types.ts` + `src/webui/schemas_v1.py`。冲突时以契约为准
...
| 3 | Activity Runs 页 | **不做** | Activity 只 Safety decisions 一 tab；跨 conv run 列表留 v3 |
...
| 8 | MCP 绑定粒度 | **整服务器**（`enabled_tools=null`）| per-tool 白名单 UI 留 v3 |
| 9 | `agent_role` 暴露 | **不暴露**（默认 `agent`）| super_agent A2A 编排是进阶概念 |
| 10 | Channel 在会话列表 | **不区分** | v1 全默认 web channel |
| 11 | `Bindings` 独立路由 | **废弃**（current placeholder） | 绑定收进 coworker 详情页 |
| 12 | `max_concurrent` 暴露 | **不暴露**（默认 2）| 同 agent_role |
...
| v2-C | Activity shell + visual polish + token lint | ~600 |
| **合计** | | **~2600** |
...
| v2-C | Activity + polish | **done** | 2026-05-23 | 3 commits on feat/ui-v2; v2 cycle 完工 — full retro in session Findings (LOC 4.17×, 2 refresh / 3 session). v3 blocker: user-scoped WS subject (backend chore) |
...
5. `contracts/openapi.yaml` + `web/src/api/generated/types.ts` — 契约
