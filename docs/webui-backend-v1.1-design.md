...
**L6 强约束**：所有 `created_by_user_id` 必须 `NULLABLE`。audit 表（`safety_rules_audit.actor_user_id`）写入路径用 helper：
...
| 3 | Bootstrap fast-path（多 user 走 §5.2.1）| 多 user 端到端跑（发起者 != 操作者）|
| 4 | Bootstrap fast-path | — |

#### 5.2.1 多 user smoke：方案 A — 扩展 bootstrap token 支持 user map

bootstrap fast-path 当前只产 `user_id="bootstrap"`。Phase 3 的多 user 业务需要发起者 != 操作者，所以扩展：
...
Phase 3 smoke 在方案 A 多 bootstrap user 下跑：alice 创建 skill -> bob 编辑，验 actor_user_id 落表正确。
...
| 3 | **alice 创建 skill / bob 编辑**（方案 A）；audit FK actor_user_id 落真 user UUID；skill 投影到容器 tmpfs；SKILL.md 受保护 |
...
INV-6  runs.{status, completed_at, usage} 在每条终止路径都被 UPDATE
       test: test_run_state_machine_all_paths（枚举 WS / cancel / schedule /
             container_crash / reauth_required）
```
