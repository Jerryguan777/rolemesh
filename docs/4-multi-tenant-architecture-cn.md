...
- 多个**组织**（租户）共享同一基础设施
- 每个租户有多个 **AI 协作者（Coworker）**（运营 AI、客服 AI 等）
- 同一类型的多个协作者（每条产品线一个、每个区域一个等）
...
协调器在 `src/rolemesh/db/_pool.py` 中提供两个连接 helper：
...
- **Agent 权限**是一个 4 字段模型（`data_scope`、`task_schedule`、`task_manage_others`、`agent_delegate`），由每个 Coworker 携带。它们在 IPC 强制时控制 agent 能做什么。这里的 IPC 契约——payload 携带 `tenantId + coworkerId`，由协调器查找权威权限——记录在 [`nats-ipc-architecture.md`](nats-ipc-architecture.md)。
...
5. **输出通道之间的去重**：agent 拥有两条输出路径——`send_message`（即时，通过 IPC）与结果流（最终，通过 NATS）。协调器追踪通过 IPC 发送的文本，并在结果流中跳过重复。
...
协调器在启动时运行 `_create_schema`；滚动部署可行，因为每次变换都是幂等的。
...
- [`nats-ipc-architecture.md`](nats-ipc-architecture.md) —— 协调器与 Agent 容器之间的 IPC 层
