...
startup assertion  core/config.py   APPROVAL_TIMEOUT < IDLE_TIMEOUT + 30_000   否则拒绝启动
```
配合 `IDLE_TIMEOUT = 1_800_000 ms` 与容器 watchdog 下限 `max(config_timeout, IDLE_TIMEOUT + 30_000)`,该断言保证审批 await 总是先于容器 watchdog 触发 —— watchdog 永远抢不掉一个审批。
...
**运营**:每个 pending 审批占住一个容器 ≤ `APPROVAL_TIMEOUT`;`MAX_CONCURRENT_CONTAINERS` / `GLOBAL_MAX_CONTAINERS` 必须留余量(被接受的取舍;触顶则 log)。
