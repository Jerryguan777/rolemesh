...
def test_lossy_partial_copy_would_have_dropped_these_fields() -> None:
    ...
    # Permissions: the lossy version reset to the least-privilege default
    # AgentPermissions (all-False, so agent_delegate=False). The populated
    # fixture sets agent_delegate=True, so a reset surfaces here.
    assert r.permissions is not None
    assert r.permissions.agent_delegate is True, (
        "Permissions look reset to defaults — partial copy regression."
    )
