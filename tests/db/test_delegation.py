...
async def test_child_conv_is_created_with_parent_link() -> None:
    """create_child_conversation stamps parent_conversation_id and the
    deterministic internal chat_id.

    The "child never enters _state" invariant (handbook §6 Step 2.4 ★)
    is enforced on current main by the ``parent_conversation_id IS NULL``
    list filter (see ``tests/core/test_loader_excludes_children.py``),
    not by the removed ``requires_trigger`` column.
    """
