...
def patch_jwks(public_jwk):
    ...
    with patch("rolemesh.auth.oidc.jwks.httpx.AsyncClient", _factory):
        yield
...
@pytest.fixture
def mock_db():
    """Patch all rolemesh.db functions used by OIDCAuthProvider."""
    ...
    with (
        ...
        patch("rolemesh.db.get_user", AsyncMock(side_effect=get_user)),
    ):
        yield state
