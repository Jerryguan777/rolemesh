...
class TestCreateRoleMeshTools:
    ...
    def test_default_omits_gated_tools(self) -> None:
        ...
        # must drop all gated categories: send_message, the delegation
        # pair (delegate_to_agent / list_agents), and the six task
        # lifecycle tools. Every tool in TOOL_DEFINITIONS currently
        # belongs to one of those gated categories, so the default
        # registry is EMPTY. Pinning this protects the v1.5 sub-chip
        # display contract — a specialist must not surface
        ...
        # No ungated tools currently exist. If a new ungated tool
        # lands, this assertion fails and forces a deliberate decision
        # about gating.
        assert tool_names == set()

    def test_register_delegation_only_adds_delegation_pair(self) -> None:
        # Frontdesks (agent_delegate=True) must see both
        # delegate_to_agent AND list_agents; a config that turned on
        # agent_delegate without registering list_agents would leave
        # the LLM unable to refresh a stale catalog.
        ctx = _make_ctx()
    ...
    def test_flags_are_independent(self) -> None:
        ...
        # Since there are no ungated tools, the two registries are
        # disjoint.
        ctx = _make_ctx()
        delegation_only = {
        ...
        assert delegation_only & task_only == set()
