...
async def _seed_world(
    *,
    ...
    parent_conv = await create_conversation(
        ...
        # Frontdesks are user-facing — leave requires_trigger=True.
        # The handbook §6 Step 2.5 invariant says child convs must NOT
        # be requires_trigger=True; parent convs can be.
...
async def test_happy_path_returns_success_and_creates_audit_row() -> None:
    ...
    Asserts: child conv created with parent_conversation_id set AND
    requires_trigger=False (handbook §6 Step 2.5 invariant); audit row
    in status='success'; response carries metadata + isError=false."""
    s = await _seed_world()
    text = AgentOutput(
    ...
    # Child conv must NOT have requires_trigger=True; otherwise the
    # _message_loop would pick it up (handbook §6 Step 2.5).
    child_id = metadata["childConversationId"]
    async with admin_conn() as conn:
        child_row = await conn.fetchrow(
            "SELECT requires_trigger, parent_conversation_id "
            "FROM conversations WHERE id = $1::uuid",
            child_id,
        )
    assert child_row is not None
    assert child_row["requires_trigger"] is False, (
        "child conv must be requires_trigger=False — otherwise "
        "_message_loop will sweep it and break the §6 Step 2.5 invariant"
    )
