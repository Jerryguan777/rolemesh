...
async def test_executor_invokes_render_catalog_only_for_frontdesk(
    ...
    # Specialist — no call.
    sp = _cw(tenant_id="t", name="Trading", folder="trading", agent_role="agent")
...
def test_a6_coworker_state_round_trip_preserves_is_frontdesk_flag() -> None:
    ...
    assert out.agent_role == "super_agent"
    assert out.status == "active"
    assert out.permissions is not None
    assert out.permissions.agent_delegate is True  # super_agent default
