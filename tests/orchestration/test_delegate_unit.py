...
that exercise validation paths and event-merge precedence:
  ...
  - #7  target is frontdesk rejected
...
def test_resolve_target_matches_by_folder() -> None:
    ...
    tr = _cw(tenant_id=tenant, name="Trading", folder="trading")
...
def test_resolve_target_rejects_cross_tenant() -> None:
    ...
    a = str(uuid.uuid4())
    b = str(uuid.uuid4())
    cw = _cw(tenant_id=b, name="Other", folder="other")
...
def test_build_agent_input_depth_increments() -> None:
    ...
    tenant = str(uuid.uuid4())
    target = _cw(tenant_id=tenant, name="T", folder="t")
    caller = _cw(tenant_id=tenant, name="F", folder="f")
    inp = _build_agent_input(
...
def test_map_output_to_response_success() -> None:
    target = _cw(tenant_id="t", name="Trading", folder="trading")
    out = AgentOutput(
